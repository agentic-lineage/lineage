...
- `lineage workflow run <workflow-name> <claude|codex|aider> --dry-run` narrows the
  launch plan to one exported workflow.

The current adapters focus on Claude, Codex, and Aider. The package shape stays plain so
...
lineage run <claude|codex|aider> [--dry-run] [--yes] [-- provider args...]
lineage workflow run <workflow-name> <claude|codex|aider> [--dry-run] [--yes] [-- provider args...]
