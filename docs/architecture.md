...
- The runtime resolves enabled packages and builds a provider plan through an explicit adapter boundary; launchable providers start after materialization, while project-scoped materialization-only providers such as Cline do not require or perform a launch.
...
- Provider shims let normal launchable commands such as `claude` or `codex` enter the Lineage runtime first, then hand off to the real provider binary. Cline is project-scoped and config/materialization-only: it is not shimmed, does not launch a CLI or GUI, and its global MCP configuration is not written.
