...
- Provider shims let a normal command such as `claude`, `codex`, or `cline` enter the Lineage runtime first, then hand off to the real provider binary. Cline is config/materialization-only: its global MCP configuration is not written.
