...
- Enabling a package doesn't just plan a launch — running a provider materializes it: skills are staged into that provider's own discovery directory (e.g. `.claude/skills/` or `.windsurf/rules/`), and a generated section is written into its context file, tracked per-provider so re-running stays idempotent. This is gated behind an explicit confirmation the first time it would actually change anything.
...
- Provider shims let a normal command such as `claude` or `codex` enter the Lineage runtime first, then hand off to the real provider binary. Windsurf is a known provider for project-scoped config/materialization only: it is not shimmed, its GUI is not launched by Lineage, and global MCP configuration is not written.
