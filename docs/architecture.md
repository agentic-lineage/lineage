...
Package instruction content — skills, workflows, agents, policies, adapters, and `setup.files[].template` — is separately scanned for risky agent-instruction patterns: prompt override, broad local reads, network/credential exfiltration, silent destructive operations, credential/session-data collection, and disable-safety language. Findings carry a hard-stop/warning severity split and are surfaced at validate, publish, inspect, and enable time; see [0016](decisions/0016-instruction-risk-scanning-severity-and-scope.md). Like the secret scan, this is a practical risk signal from a small documented pattern list, not a complete prompt-injection defense.
